...
# SMOOTHING:
# Original alpha=0.7 was TOO RESPONSIVE and JITTERY.
# New alpha=0.2 is MUCH SMOOTHER, giving 80% weight to history.
SMOOTHING_ALPHA = 0.2
smoothed_yaw = None
smoothed_pitch = None

# MAPPING:
# Original ranges were too narrow, making 1 degree of jitter = huge pixel jump.
# New ranges are wider, making controls less sensitive and more stable.
YAW_RANGE = [-45, 45]  # [Min angle Left, Max angle Right]
PITCH_RANGE = [-35, 25]  # [Min angle Down, Max angle Up]

# CLICK LOGIC:
# We now use a state machine for clicks,
# triggering on the *event* of a fist closing, not the *state* of it being held.
...
# Camera parameters (will be set dynamically)
FOCAL_LENGTH = 1
CENTER = (0, 0)
...
def estimate_head_pose(img_rgb):
    ...
    # Re-calculate camera params for this frame size
    FOCAL_LENGTH = iw
    CENTER = (iw / 2, ih / 2)
    ...
    # Return raw data (smoothing will be applied in main loop)
...
while cap.isOpened():
    ...
    # Flip the frame for a natural mirror-like view
        ...
        for hand_landmarks in hand_results.multi_hand_landmarks:
            ...
            try:
                is_fist = (
                    ...
                    # Fix: Corrected typo from HandLandkey to HandLandmark
                    landmarks[mp_hands.HandLandmark.PINKY_TIP].y > landmarks[mp_hands.HandLandmark.PINKY_PIP].y
                )
            except:
                is_fist = False # Fail safe if landmarks are not clear
            ...
            # --- NEW Event-Based Click and Scroll Logic ---
            if is_fist:
                ...
                if scroll_prev_y is not None:
                    ...
                    if abs(scroll_delta) > 0.01: # Movement threshold
                        if scroll_delta > 0:
                ...
                scroll_prev_y = index_tip_y # Update previous Y
    ...
    if gaze_data:
        ...
        # --- MOUSE MOVEMENT ---
        # Only move the mouse if no gesture is being actively performed
        if not gesture_detected:
            if status_text == "":
                status_text = "Moving"

            # Map the smoothed angles to screen coordinates
            # Note: We use your original [screen_width - 1, 0] for INVERTED X-axis
            target_x = np.interp(smoothed_yaw, YAW_RANGE, [screen_width - 1, 0])
            target_y = np.interp(smoothed_pitch, PITCH_RANGE, [screen_height - 1, 0])import cv2
import numpy as np
import mediapipe as mp
import pyautogui
import time
...
hands.close()
            ...
            # Clip values to ensure they are within screen bounds
            target_x = np.clip(target_x, 0, screen_width - 1)
            target_y = np.clip(target_y, 0, screen_height - 1)
